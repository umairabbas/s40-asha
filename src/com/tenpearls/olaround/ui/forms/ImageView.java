...
public class ImageView extends Form {
    ...
    public ImageView(final Image image, ImageGrid imageGrid) {
        this.image = image;
    	...
    	Command selectPhotorCmd = new Command(ApplicationConstants.SELECT_PHOTO_COMMAND) {
    		 public void actionPerformed(ActionEvent e) {
    				System.out.println("OnSelectPhoto");
    				OlaroundMidlet.getInstance().destoryGalleryForm();
    				OlaroundMidlet.getInstance().showSignUpStep1Form(null, true, image);
             }
    	};
		addCommand(selectPhotorCmd);
		setDefaultCommand(selectPhotorCmd);
